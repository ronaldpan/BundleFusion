...
RGBDSensor* getRGBDSensor()
{
	...
	if (GlobalAppState::get().s_sensorIdx == 9) {
		...
		g_sensor = new StructureCore;
		return g_sensor;
#else
		throw MLIB_EXCEPTION("Requires STRUCTURE_CORE macro");
#endif
	...
	if (GlobalAppState::get().s_sensorIdx == 10) {
			...
			return g_sensor;
#else
			throw MLIB_EXCEPTION("Requires Astra macro");
#endif
	}
...
void bundlingThreadFunc() {
	...
	while (1) {
		...
		if (g_RGBDSensor->isReceivingFrames()) {
			int currProcessedFrame = g_bundler->getCurrProcessedFrame();
			if (currProcessedFrame>0 && currProcessedFrame % GlobalBundlingState::get().s_submapSize == 0) { // stop solve
				if (tOpt.joinable()) {
					tOpt.join();
				}
			}
			if (currProcessedFrame>1 && currProcessedFrame % GlobalBundlingState::get().s_submapSize == 1) { // start solve，每个chunk优化执行优化
		...
		g_bundler->processInput();						//perform sift and whatever,处理frame
...
int main(int argc, char** argv)
{
	...
	try {
		...
		//dualGPU.setDevice(DualGPU::DEVICE_BUNDLING); //只用一个GPU
		...
		//dualGPU.setDevice(DualGPU::DEVICE_RECONSTRUCTION);	//main gpu //只用一个GPU

		startDepthSensing(g_bundler, getRGBDSensor(), g_imageManager); //start depthSensing render loop
...
#if (0)
		//==panrj
		if (GlobalAppState::get().s_SDF_CPUEnable) { //在CPU中提取mesh
			cout << "在CPU中提取mesh..." << endl;
			std::vector<mat4f> trajectory2;
			((SensorDataReader*)g_RGBDSensor)->getTrajectory(trajectory2);
			//g_bundler->getTrajectoryManager()->getOptimizedTransforms(trajectory2);
			int numFrames = trajectory2.size();
			//传入g_imageManager对象对depthsensing中的深度相机参数初始化
			startDepthSensing2(g_imageManager);
			...
			{
				cout << "正在处理第" << i << "帧......" << endl;
				FrameRender(i, trajectory2);
			}
			std::cout << "所有帧处理完毕......" << endl;
			std::cout << "正在生成模型......" << endl;
			ExtractIsoSurfaceMC_CPU();
			std::cout << "生成模型成功......" << endl;
		}
		//==panrj
#endif

#ifdef RUN_MULTITHREADED 
		g_bundler->setExitBundlingThread();
