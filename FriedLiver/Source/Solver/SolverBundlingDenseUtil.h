...
// camera functions
///////////////////////////////////////////////////////////////////
__inline__ __device__ bool computeAngleDiff(const float4x4& transform, float cosAngleThresh)
{
	float3 x = normalize(make_float3(1.0f, 1.0f, 1.0f));
	float3 v = transform.getFloat3x3() * x;
	//-----------panrj
	return (dot(x, v)) > cosAngleThresh; //¼Ð½ÇÔÚ[-angleThresh,+angleThresh]Ö®¼ä
	//-----------panrj

	//float angle = acos(clamp(dot(x, v), -1.0f, 1.0f)); //[0,pi]
	//return  (fabs(angle) < angleThresh);
}
