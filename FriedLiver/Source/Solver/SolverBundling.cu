...
#endif
		//if (!computeAngleDiff(transform, 1.0f)) return; //~60 degrees //TODO HERE ANGIE, 60.0/180*PI
		//if (!computeAngleDiff(transform, 0.8f)) return; //~45 degrees
		//if (!computeAngleDiff(transform, 0.52f)) return; //~30 degrees,Ô­À´
		if (!computeAngleDiff(transform, 0.5f)) return; //cos(30degrees)=0.866f, panrj
...
bool BuildDenseSystem(const SolverInput& input, SolverState& state, SolverParameters& parameters, CUDATimer* timer)
{
	...
	if (parameters.useDenseDepthAllPairwise) 
		FindImageImageCorr_Kernel<true> << < gridImImOverlap, THREADS_PER_BLOCK_DENSE_OVERLAP >> >(input, state, parameters);
	else									 
		FindImageImageCorr_Kernel<false> << < gridImImOverlap, THREADS_PER_BLOCK_DENSE_OVERLAP >> >(input, state, parameters);
#ifdef _DEBUG
	cutilSafeCall(cudaDeviceSynchronize());
	cutilCheckMsg(__FUNCTION__);
#endif
...
extern "C" void solveBundlingStub(SolverInput& input, SolverState& state, SolverParameters& parameters, SolverStateAnalysis& analysis, float* convergenceAnalysis, CUDATimer *timer)
{
	...
	for (unsigned int nIter = 0; nIter < parameters.nNonLinearIterations; nIter++)
	{
		...
		if (parameters.useDense) 
			parameters.useDense = BuildDenseSystem(input, state, parameters, timer); //don't solve dense if no overlapping frames found
