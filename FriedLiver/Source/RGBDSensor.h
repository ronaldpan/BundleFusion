...
public:
	...
	//! Get the extrinsic camera matrix of the depth sensor
	...
	void saveRecordedFramesToFile(std::string& filename, const std::vector<mat4f>& trajectory, bool overwriteExistingFile = false);
...
public:
	mat4f m_pose;
	bool m_hasPoseSensor{ false };
	UINT64 m_timeStampColor{ 0 };
	UINT64 m_timeStampDepth{ 0 };
};
