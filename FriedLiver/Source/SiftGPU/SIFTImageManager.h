...
public:
	...
	void reset() {
		...
		m_numKeyPointsPerImagePrefixSum.clear(); m_numKeyPointsPerImagePrefixSum.push_back(0);
		m_numKeyPoints = 0;
		...
		m_validImages[0] = 1; // first is valid 强制frame0有效
	}
	...
	unsigned int getCurrentFrame() const { return m_iCurrentImage; }
	void setCurrentFrame(unsigned int idx) { m_iCurrentImage = idx; }
