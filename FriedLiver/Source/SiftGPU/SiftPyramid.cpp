...
void SiftPyramid::GetFeatureVectorCUDA(float* d_descriptor, unsigned int maxNumKeyPoints)
{
	if (GlobalUtil::_EnableDetailedTimings) {
		_timer->startEvent("ConvertDescriptorToFloat");
	}
	int numDescriptors = (maxNumKeyPoints == (unsigned int)-1) ? _featureNum : std::min(_featureNum, (int)maxNumKeyPoints);
	if (numDescriptors > 0) 	cutilSafeCall(cudaMemcpy(d_descriptor, d_outDescriptorList, sizeof(float) * 128 *numDescriptors, cudaMemcpyDeviceToDevice));
	if (GlobalUtil::_EnableDetailedTimings) {
		_timer->endEvent();
	}
}
