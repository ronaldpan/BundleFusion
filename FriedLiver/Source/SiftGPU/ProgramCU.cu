...
#pragma unroll
	...
	if (threadIdx.x == 0)
	{
		 // acosf：返回[0, π]，error occurs if arg is outside the range[-1.0; 1.0]，NaN is returned.
		float dist = acosf(min(dotmax[0] * 0.000003814697265625f, 1.0f)); //2^(-18)=1.0/(2^9*2^9)
...
void __global__  ColMatch_Kernel(int height, int num2, float distmax, float ratiomax, 
	const int* d_rowResult, float* d_matchDistances, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset)
{
...
void ProgramCU::GetColMatch(CuTexImage* texCRT, float distmax, float ratiomax, CuTexImage* rowMatch, 
	float* d_matchDistances, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset, int* numMatches)
{
	...
	ColMatch_Kernel << <grid, block >> >(height, num2, distmax, ratiomax, (int*)rowMatch->_cuData, 
		d_matchDistances, d_outKeyPointIndices, d_outMatchDistances, d_numMatches, keyPointOffset);
...
void __global__  ConvertDescriptorToUChar_Kernel(const float* d_descriptorsFloat, unsigned int numDescriptorElements, unsigned char* d_descriptorsUChar)
{
	...
	if (idx < numDescriptorElements) {
		d_descriptorsUChar[idx] = int(512 * d_descriptorsFloat[idx] + 0.5); //乘上 2^9, 舍入，取低字节。大于0.5的都转换成255
	}
}
...
void __global__  GetOpenCVMatch_Kernel(cv::cuda::PtrStepSz<int> gm1, cv::cuda::PtrStepSz<float> gm2, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset)
{
	int Idx = blockDim.x * blockIdx.x + threadIdx.x;
	if (Idx < gm1.cols && gm1(0, Idx) != -1)
	{
		{
			//匹配信息赋值
			int addr = atomicAdd(d_numMatches, 1);
			if (addr < MAX_MATCHES_PER_IMAGE_PAIR_RAW)
			{
				d_outKeyPointIndices[addr] = make_uint2(Idx + keyPointOffset.x, gm1(0, Idx) + keyPointOffset.y);
				d_outMatchDistances[addr] = gm2(1, Idx);
			}
		}
	}
}

void ProgramCU::GetOpenCVMatch(cv::cuda::PtrStepSz<int> gm1, cv::cuda::PtrStepSz<float> gm2, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset)
{
	const unsigned int threadsPerBlock = 32;
	dim3 grid((gm1.cols + threadsPerBlock - 1) / threadsPerBlock);
	dim3 block(threadsPerBlock, 1, 1);
	cutilSafeCall(cudaMemset(d_numMatches, 0, sizeof(int)));

	GetOpenCVMatch_Kernel << <grid, block >> > (gm1, gm2, d_outKeyPointIndices, d_outMatchDistances, d_numMatches, keyPointOffset);

	ProgramCU::CheckErrorCUDA(__FUNCTION__);
}

void __global__  GetOpenCVKnnMatch_Kernel(cv::cuda::PtrStepSz<int2> gm1, cv::cuda::PtrStepSz<float2> gm2, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset)
{
	int Idx = blockDim.x * blockIdx.x + threadIdx.x;
	if (Idx < gm1.cols && gm1(0, Idx).x != -1)
	{
		{
			//匹配信息赋值
			int addr = atomicAdd(d_numMatches, 1);
			if (addr < MAX_MATCHES_PER_IMAGE_PAIR_RAW)
			{
				d_outKeyPointIndices[addr] = make_uint2(Idx + keyPointOffset.x, gm1(0, Idx).x + keyPointOffset.y);
				d_outMatchDistances[addr] = gm2(1, Idx).x;
			}
		}
	}
}

void ProgramCU::GetOpenCVKnnMatch(cv::cuda::PtrStepSz<int2> gm1, cv::cuda::PtrStepSz<float2> gm2, uint2* d_outKeyPointIndices, float* d_outMatchDistances, int* d_numMatches, uint2 keyPointOffset)
{
	const unsigned int threadsPerBlock = 32;
	dim3 grid((gm1.cols + threadsPerBlock - 1) / threadsPerBlock);
	dim3 block(threadsPerBlock, 1, 1);
	...
	GetOpenCVKnnMatch_Kernel << <grid, block >> > (gm1, gm2, d_outKeyPointIndices, d_outMatchDistances, d_numMatches, keyPointOffset);
