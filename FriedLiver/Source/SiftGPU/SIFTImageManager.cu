...
void __global__ FilterKeyPointMatchesCU_Kernel(
	unsigned int curFrame, 	unsigned int startFrame,
	...
	float maxKabschRes2)
{
	...
	if (tidx == 0) 	{
		float4x4 trans;
		numFilteredMatches = filterKeyPointMatches(d_keyPointsGlobal, matchKeyPointIndices, matchDistances, numMatches,
	...
	//write results back
	if (tidx == 0) { d_numFilteredMatchesPerImagePair[imagePairIdx] = numFilteredMatches; }
...
void SIFTImageManager::FilterKeyPointMatchesCU(unsigned int curFrame, unsigned int startFrame, unsigned int numFrames, 
	const float4x4& siftIntrinsicsInv, unsigned int minNumMatches, float maxKabschRes2) {
...
//we launch 1 thread for two array entries
void __global__ FilterMatchesByDenseVerifyCU_Kernel(unsigned int curImageIdx, unsigned int startFrame, unsigned int imageWidth, unsigned int imageHeight, 
	const float4x4 intrinsics,
	int* d_currNumFilteredMatchesPerImagePair, const float4x4* d_currFilteredTransforms, const float4x4* d_currFilteredTransformsInv, 
	const CUDACachedFrame* d_cachedFrames,
...
void SIFTImageManager::FilterMatchesByDenseVerifyCU(unsigned int curFrame, unsigned int startFrame, unsigned int numFrames, 
