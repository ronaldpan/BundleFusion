...
SIFTImageManager::SIFTImageManager(unsigned int maxImages /*= 500*/, unsigned int maxKeyPointsPerImage /*= 4096*/)
{
	...
	m_numKeyPointsPerImagePrefixSum.push_back(0); //先放1个0
}
...
unsigned int SIFTImageManager::getNumKeyPointsPerImage(unsigned int imageIdx) const
{
	...
	return m_numKeyPointsPerImagePrefixSum[imageIdx + 1] - m_numKeyPointsPerImagePrefixSum[imageIdx];
}
...
void SIFTImageManager::finalizeSIFTImageGPU(unsigned int numKeyPoints)
{
	...
	m_numKeyPoints += numKeyPoints; //keyPoints总数
	//m_numKeyPointsPerImage.push_back(numKeyPoints);
	m_numKeyPointsPerImagePrefixSum.push_back(m_numKeyPoints);
	m_bFinalizedGPUImage = true;
	m_iCurrentImage = (unsigned int)m_SIFTImagesGPU.size() - 1;

	//assert(getNumImages() == m_numKeyPointsPerImage.size());
	assert(getNumImages() == m_numKeyPointsPerImagePrefixSum.size()-1);
}
...
void SIFTImageManager::loadFromFile(const std::string& s)
{
	...
	m_numKeyPointsPerImagePrefixSum.resize(numImages+1);
...
void SIFTImageManager::computeTracks(const std::vector<float4x4>& trajectory, const std::vector<EntryJ>& correspondences, const std::vector<uint2>& correspondenceKeyIndices,
	...
	for (unsigned int i = 0; i < numImages; i++) {
		...
		for (unsigned int k = 0; k < m_numKeyPointsPerImagePrefixSum[i+1]- m_numKeyPointsPerImagePrefixSum[i]; k++) {
	...
	const float4x4& colorIntrinsicsInv) const
{
	...
	for (unsigned int t = 0; t < tracks.size(); t++) {
		...
		if (num > 0) {
			pos /= (float)num; //average track locations，取平均点
	...
	if (curKeys.size() > m_maxKeyPointsPerImage) {//点过多时，深度小的优先
		std::sort(curKeys.begin(), curKeys.end(), [](const SIFTKeyPoint& left, const SIFTKeyPoint& right) {
			return left.depth < right.depth;
		});
	}
	SIFTImageGPU& cur = global->createSIFTImageGPU(); //新的关键帧 keyframe
...
unsigned int SIFTImageManager::filterFrames(unsigned int curFrame, unsigned int startFrame, unsigned int numFrames)
{
	...
	MLIB_CUDA_SAFE_CALL(cudaMemcpy(currNumFilteredMatchesPerImagePair.data(), d_currNumFilteredMatchesPerImagePair + startFrame, 
		sizeof(unsigned int) * currNumFilteredMatchesPerImagePair.size(), cudaMemcpyDeviceToHost));

	int bConnected = 0;
	unsigned int lastMatchedFrame = (unsigned int)-1;
	for (int i = (int)numFrames-1; i >= (int)startFrame; i--) { // previous frames (reverse direction) ，反序查找，找到1个就结束
	...
	m_validImages[curFrame] = bConnected;
