...
void CUDAMarchingCubesHashSDF::create(const MarchingCubesParams& params)
{
	...
	m_data.allocate(m_params); //on GPU
	//m_data.allocate(m_params, false); // on CPU

	resetMarchingCubesCUDA(m_data); //GPU
}

void CUDAMarchingCubesHashSDF::copyTrianglesToCPU() {
	//GPU转到CPU,再转入m_meshdata
...
void CUDAMarchingCubesHashSDF::saveMesh(const std::string& filename, const mat4f *transform /*= NULL*/, bool overwriteExistingFile /*= false*/)
{
	...
	if (!overwriteExistingFile) { //修改文件名中的数字编号
...
//==panrj
void CUDAMarchingCubesHashSDF::extractIsoSurfaceCPU(const HashDataStruct& hashData, const HashParams& hashParams, RayCastData& rayCastData)
{
	//reset();
	m_params.m_numOccupiedSDFBlocks = hashParams.m_numOccupiedBlocks;
	m_params.m_maxCorner = MatrixConversion::toCUDA(vec3f(0.0f, 0.0f, 0.0f));
	m_params.m_minCorner = MatrixConversion::toCUDA(vec3f(0.0f, 0.0f, 0.0f));
	m_params.m_boxEnabled = false;
	//m_data.updateParams(m_params);
	MarchingCubesData cpuData;
	cpuData.allocate(m_params, false);//m_data;// .copyToCPU();
	cpuData.updateParams(m_params);
	//HashDataStruct		  cpuHashData = hashData.copyToCPU(hashParams);//原来数据在GPU上
	HashDataStruct		  cpuHashData = hashData;
	std::cout << "sdf块数:" << m_params.m_hashNumBuckets*m_params.m_hashBucketSize << std::endl;

	/*20210329 Jing 在循环外取cuda中常量内存的值*/
	size_t size = sizeof(HashParams);
	memcpy(&cpuData.hashParams_mc, &hashParams, size);
	memcpy(&cpuHashData.hashParams_hd, &hashParams, size);
	memcpy(&rayCastData.hashParams_rc, &hashParams, size);
	//std::cout << "sdf块数:" << rayCastData.hashParams_rc.m_hashNumBuckets*rayCastData.hashParams_rc.m_hashBucketSize << std::endl;
	//std::cout << "mc" << (cpuData.hashParams_mc) << "hd"<<cpuHashData.hashParams_hd << "rc" <<rayCastData.hashParams_rc << std::endl;

	for (unsigned int sdfBlockId = 0; sdfBlockId < m_params.m_hashNumBuckets*m_params.m_hashBucketSize; sdfBlockId++) {//m_params.m_numOccupiedSDFBlocks
		for (int x = 0; x < hashParams.m_SDFBlockSize; x++) {
			for (int y = 0; y < hashParams.m_SDFBlockSize; y++) {
				for (int z = 0; z < hashParams.m_SDFBlockSize; z++) {

					const HashEntry& entry = cpuHashData.d_hash[sdfBlockId];
					if (entry.ptr != FREE_ENTRY) {
						int3 pi_base = cpuHashData.SDFBlockToVirtualVoxelPosCPU(entry.pos);
						//std::cout << pi_base.x <<"========"<<pi_base.y<<"========"<<pi_base.z<<std::endl;
						int3 pi = pi_base + make_int3(x, y, z);
						float3 worldPos = cpuHashData.virtualVoxelPosToWorldCPU(pi);

						cpuData.extractIsoSurfaceAtPositionCPU(worldPos, cpuHashData, rayCastData);
					}

				} // z
			} // y
		} // x
		//std::cout << "left is:" << m_params.m_hashNumBuckets*m_params.m_hashBucketSize - sdfBlockId << std::endl;
	} // sdf block id
