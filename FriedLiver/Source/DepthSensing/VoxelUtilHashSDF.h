...
#ifndef MINF //负的，最小float（指数最大128，尾数为1.0）
#define MINF __int_as_float(0xff800000)
#endif

#ifndef PINF //正的，最大float
...
struct HashEntry 
{
	...
	__host__ __device__ void operator=(const struct HashEntry& e) {
		//((int*)this)[0] = ((const int*)&e)[0];
		//((int*)this)[1] = ((const int*)&e)[1];
		//((int*)this)[2] = ((const int*)&e)[2];
		//((int*)this)[3] = ((const int*)&e)[3];
		//((int*)this)[4] = ((const int*)&e)[4];
		((long long*)this)[0] = ((const long long*)&e)[0]; //long long is an integer type which is at least 64-bit(8B) wide.
...
struct Voxel {
	...
	__host__ __device__ void operator=(const struct Voxel& v) {
...
struct HashDataStruct {
	...
	void allocate(const HashParams& params, bool dataOnGPU = true) {
		m_bIsOnGPU = dataOnGPU;//这种机制保证了更新hash字段都是在GPU上进行
		if (m_bIsOnGPU) {
			std::cout << "为gpu的hash分配空间" << std::endl;
			...
			std::cout << "为cpu的hash分配空间" << std::endl;
	...
	void updateParams(const HashParams& params) {
		if (m_bIsOnGPU) {
			updateConstantHashParams(params);//GPU
		} 
	}
	...
	__host__
	HashDataStruct copyToCPU(HashParams params) const { //在CUDAMarchingCubesHashSDF::extractIsoSurfaceCPU中call
		//HashParams params; //没有初始化?
	...
	//==panrj
	HashParams hashParams_hd;
	//新增从SDF块到体素的坐标转换，host端
	__host__
		int3 SDFBlockToVirtualVoxelPosCPU(const int3& sdfBlock) const {
		return sdfBlock * SDF_BLOCK_SIZE;
	}
	//新增体素到世界坐标的转换，host端
	__host__
		float3 virtualVoxelPosToWorldCPU(const int3& pos) const {
		//HashParams b_hashParams;
		//size_t size=sizeof(HashParams);
		//cudaGetSymbolSize(&size, &c_hashParams);
		//cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);
		return make_float3(pos)*(hashParams_hd).m_virtualVoxelSize;
	}
	//world世界坐标系转回SDF块坐标系，host端
	__host__
		int3 worldToSDFBlockCPU(const float3& worldPos) const {
		return virtualVoxelPosToSDFBlockCPU(worldToVirtualVoxelPosCPU(worldPos));
	}
	__host__
		int3 virtualVoxelPosToSDFBlockCPU(int3 virtualVoxelPos) const {
	...
	__host__
		int3 worldToVirtualVoxelPosCPU(const float3& pos) const {
		//const float3 p = pos*g_VirtualVoxelResolutionScalar;
		/*HashParams b_hashParams;
		size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		const float3 p = pos / (hashParams_hd).m_virtualVoxelSize;
		return make_int3(p + make_float3(sign(p))*0.5f);
	}
	__host__
		void deleteVoxelCPU(Voxel& v) const {
		v.color = make_uchar4(0, 0, 0, 0);
		v.weight = 0.0f;
		v.sdf = 0.0f;
	}
	__host__
		void deleteVoxelCPU(uint id) {
		deleteVoxelCPU(d_SDFBlocks[id]);
	}

	__host__
		Voxel getVoxelCPU(const float3& worldPos) const {
		HashEntry hashEntry = getHashEntryCPU(worldPos);//291行
		Voxel v;
		if (hashEntry.ptr == FREE_ENTRY) {
			deleteVoxelCPU(v);//244行
		}
		else {
			int3 virtualVoxelPos = worldToVirtualVoxelPosCPU(worldPos);//238行
			v = d_SDFBlocks[hashEntry.ptr + virtualVoxelPosToLocalSDFBlockIndexCPU(virtualVoxelPos)];//278行
		}
		return v;
	}

	__host__
		uint linearizeVoxelPosCPU(const int3& virtualVoxelPos)	const {
		return
	...
	__host__
		int virtualVoxelPosToLocalSDFBlockIndexCPU(const int3& virtualVoxelPos) const {
		...
		return linearizeVoxelPosCPU(localVoxelPos);
	}

	__host__
		HashEntry getHashEntryCPU(const float3& worldPos) const {
		//int3 blockID = worldToSDFVirtualVoxelPos(worldPos)/SDF_BLOCK_SIZE;	//position of sdf block
		int3 blockID = worldToSDFBlockCPU(worldPos);
		return getHashEntryForSDFBlockPosCPU(blockID);
	}

	__host__
		HashEntry getHashEntryForSDFBlockPosCPU(const int3& sdfBlock) const
	{
		uint h = computeHashPosCPU(sdfBlock);			//hash bucket
		...
		size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		uint g_MaxLoopIterCount = (hashParams_hd).m_hashMaxCollisionLinkedListSize;
#pragma unroll 1 
		while (maxIter < g_MaxLoopIterCount) {
			...
			i %= (HASH_BUCKET_SIZE * (hashParams_hd).m_hashNumBuckets);	//check for overflow
	...
	__host__
		uint computeHashPosCPU(const int3& virtualVoxelPos) const {
		...
		size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		int res = ((virtualVoxelPos.x * p0) ^ (virtualVoxelPos.y * p1) ^ (virtualVoxelPos.z * p2)) % (hashParams_hd).m_hashNumBuckets;
		if (res < 0) res += (hashParams_hd).m_hashNumBuckets;
		return (uint)res;
	}
	__host__
		uint computeHashPosCPU0(const int3& virtualVoxelPos, HashParams hashParams) const {
		const int p0 = 73856093;
		const int p1 = 19349669;
		const int p2 = 83492791;
		...
		size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		int res = ((virtualVoxelPos.x * p0) ^ (virtualVoxelPos.y * p1) ^ (virtualVoxelPos.z * p2)) % (hashParams).m_hashNumBuckets;
		if (res < 0) res += (hashParams).m_hashNumBuckets;
		//printf("%d\n", res);
		return (uint)res;
	}
	__host__
		float3 worldToVirtualVoxelPosFloatCPU(const float3& pos) const {
		/*HashParams b_hashParams;
		size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		return pos / (hashParams_hd).m_virtualVoxelSize;
	}
	__host__
		float getTruncationCPU(float z, HashParams hashParams) const {
		return hashParams.m_truncation + hashParams.m_truncScale * z;
	}

	__host__
		float3 SDFBlockToWorldCPU(const int3& sdfBlock) const {
		return virtualVoxelPosToWorldCPU(SDFBlockToVirtualVoxelPosCPU(sdfBlock));
	}

	__host__
		bool isSDFBlockInCameraFrustumApproxCPU(const int3& sdfBlock, HashParams hashParams, DepthCameraParams depthCameraParams) {
		float3 posWorld = virtualVoxelPosToWorldCPU(SDFBlockToVirtualVoxelPosCPU(sdfBlock)) + hashParams.m_virtualVoxelSize * 0.5f * (SDF_BLOCK_SIZE - 1.0f);
		return DepthCameraData::isInCameraFrustumApproxCPU(hashParams.m_rigidTransformInverse, posWorld, depthCameraParams);
	}

	__host__
		void allocBlockCPU(const int3& pos, HashParams hashParams) {

		uint h = computeHashPosCPU0(pos, hashParams);				//hash bucket
		uint hp = h * HASH_BUCKET_SIZE;	//hash position 第h号bucket
		...
		unsigned int maxIter = 0;
		uint g_MaxLoopIterCount = hashParams.m_hashMaxCollisionLinkedListSize;
#pragma  unroll 1 
		while (maxIter < g_MaxLoopIterCount) {
			//offset = curr.offset;
			...
			i = idxLastEntryInBucket + curr.offset;		//go to next element in the list
			i %= (HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets);	//check for overflow
		...
		if (firstEmpty != -1) {	//if there is an empty entry and we haven't allocated the current entry before
			...
			//原子替换操作 jfwrongs
			int prevValue = d_hashBucketMutex[h];
			d_hashBucketMutex[h] = LOCK_ENTRY;
			if (prevValue != LOCK_ENTRY) {	//only proceed if the bucket has been locked
				HashEntry& entry = d_hash[firstEmpty];
				entry.pos = pos;
				entry.offset = NO_OFFSET;
				//d_heap[d_heapCounter[0]];
				entry.ptr = d_heap[d_heapCounter[0]] * SDF_BLOCK_SIZE*SDF_BLOCK_SIZE*SDF_BLOCK_SIZE;	//memory alloc
				d_heapCounter[0]--;
			}
			return;
		}
...
#pragma  unroll 1 
		while (maxIter < g_MaxLoopIterCount) {
			offset++;
			i = (idxLastEntryInBucket + offset) % (HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets);	//go to next hash element
	...
	__host__
		uint3 delinearizeVoxelIndexCPU(uint idx) const {
	...
	__host__
		void deleteHashEntryCPU(HashEntry& hashEntry) {
