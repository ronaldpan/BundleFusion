...
void DX11PhongLighting::render(ID3D11DeviceContext* pd3dDeviceContext, ID3D11ShaderResourceView* positions, ID3D11ShaderResourceView* normals, ID3D11ShaderResourceView* colors, 
	bool useMaterial, unsigned int width, unsigned int height, const vec3f& overlayColor)
{
	// save render targets
	...
	//当需要用CPU读写（GPU的）subresouce（最常用如buffer）时，就用Map()得到该subresource的pointer（D3D11_MAPPED_SUBRESOURCE*），
	//然后将D3D11_MAPPED_SUBRESOURCE::pData强制转换成CPU理解的类型（struct，class），CPU就可以对得到的struct*或class*读写了。
	// Initialize Constant Buffers,更新GPU Shader中Constant buffer，把CPU中的数据写入GPU中Constant Buffer。
	...
	pd3dDeviceContext->Unmap(s_ConstantBuffer, 0); //当调用了Map()，最后调用ID3D11DeviceContext::Unmap Method使得GPU重新获取对该resouce的读写权。
