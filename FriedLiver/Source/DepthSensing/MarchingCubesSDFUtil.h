...
struct MarchingCubesData {
	...
	//==panrj
	HashParams hashParams_mc; //拷贝cuda常量内存的c_hashParams

	//新增从hashDataStruct中根据世界坐标提取出rayCast数据的函数
	void extractIsoSurfaceAtPositionCPU(const float3& worldPos, const HashDataStruct& hashData, const RayCastData& rayCastData)
	{
		/*size_t size = sizeof(HashParams);
		cudaMemcpyFromSymbol(&b_hashParams, &c_hashParams, size, 0, cudaMemcpyDeviceToHost);*/
		const HashParams& hashParams = hashParams_mc;
		const MarchingCubesParams& params = *d_params;

		if (params.m_boxEnabled == 1) {
			if (!isInBoxAACPU(params.m_minCorner, params.m_maxCorner, worldPos)) return;//230行
		}
		...
		float3 p000 = worldPos + make_float3(M, M, M); float dist000; uchar3 color000; bool valid000 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p000, dist000, color000);//RayCastSDFUtil.h 77行
		float3 p100 = worldPos + make_float3(P, M, M); float dist100; uchar3 color100; bool valid100 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p100, dist100, color100);
		float3 p010 = worldPos + make_float3(M, P, M); float dist010; uchar3 color010; bool valid010 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p010, dist010, color010);
		float3 p001 = worldPos + make_float3(M, M, P); float dist001; uchar3 color001; bool valid001 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p001, dist001, color001);
		float3 p110 = worldPos + make_float3(P, P, M); float dist110; uchar3 color110; bool valid110 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p110, dist110, color110);
		float3 p011 = worldPos + make_float3(M, P, P); float dist011; uchar3 color011; bool valid011 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p011, dist011, color011);
		float3 p101 = worldPos + make_float3(P, M, P); float dist101; uchar3 color101; bool valid101 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p101, dist101, color101);
		float3 p111 = worldPos + make_float3(P, P, P); float dist111; uchar3 color111; bool valid111 = rayCastData.trilinearInterpolationSimpleFastFastCPU(hashData, p111, dist111, color111);
		...
		Voxel v = hashData.getVoxelCPU(worldPos);//VoxelUtilHashSDF.h 223行以下

		Vertex vertlist[12];
		if (edgeTable[cubeindex] & 1)	vertlist[0] = vertexInterpCPU(isolevel, p010, p110, dist010, dist110, v.color, v.color);//206行
		if (edgeTable[cubeindex] & 2)	vertlist[1] = vertexInterpCPU(isolevel, p110, p100, dist110, dist100, v.color, v.color);
		if (edgeTable[cubeindex] & 4)	vertlist[2] = vertexInterpCPU(isolevel, p100, p000, dist100, dist000, v.color, v.color);
		if (edgeTable[cubeindex] & 8)	vertlist[3] = vertexInterpCPU(isolevel, p000, p010, dist000, dist010, v.color, v.color);
		if (edgeTable[cubeindex] & 16)	vertlist[4] = vertexInterpCPU(isolevel, p011, p111, dist011, dist111, v.color, v.color);
		if (edgeTable[cubeindex] & 32)	vertlist[5] = vertexInterpCPU(isolevel, p111, p101, dist111, dist101, v.color, v.color);
		if (edgeTable[cubeindex] & 64)	vertlist[6] = vertexInterpCPU(isolevel, p101, p001, dist101, dist001, v.color, v.color);
		if (edgeTable[cubeindex] & 128)	vertlist[7] = vertexInterpCPU(isolevel, p001, p011, dist001, dist011, v.color, v.color);
		if (edgeTable[cubeindex] & 256)	vertlist[8] = vertexInterpCPU(isolevel, p010, p011, dist010, dist011, v.color, v.color);
		if (edgeTable[cubeindex] & 512)	vertlist[9] = vertexInterpCPU(isolevel, p110, p111, dist110, dist111, v.color, v.color);
		if (edgeTable[cubeindex] & 1024) vertlist[10] = vertexInterpCPU(isolevel, p100, p101, dist100, dist101, v.color, v.color);
		if (edgeTable[cubeindex] & 2048) vertlist[11] = vertexInterpCPU(isolevel, p000, p001, dist000, dist001, v.color, v.color);
		...
		for (int i = 0; triTable[cubeindex][i] != -1; i += 3)
		{
			...
			appendTriangleCPU(t);//238行
		}
	}

	__host__
		Vertex vertexInterpCPU(float isolevel, const float3& p1, const float3& p2, float d1, float d2, const uchar4& c1, const uchar4& c2) const
	{
	...
	__host__
		bool isInBoxAACPU(const float3& minCorner, const float3& maxCorner, const float3& pos) const
	{
	...
	__host__
		...
		//	__host__
		//#endif
		void appendTriangleCPU(const Triangle& t) {
	...
	void allocate(const MarchingCubesParams& params, bool dataOnGPU = true) {
		...
		if (m_bIsOnGPU) {
			...
			*d_numTriangles = 0;//初值未初始化导致有时候会出现一个很大的初值，在marchingcube阶段的第一步直接超过设定的最大marchingcube的三角形的值，
			//然后return了，就没有后续操作了,出现模型为空的情况
		}
	}
