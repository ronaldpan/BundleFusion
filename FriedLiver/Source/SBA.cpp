...
SBA::SBA()
{
	...
	m_localWeightsDenseColor.resize(maxNumIts, 0.1f); //原来是0.0
	...
	m_maxResidual = -1.0f;
#ifdef USE_GLOBAL_DENSE_EVERY_FRAME //每增加1个keyframe，都调用globale dense优化，会变慢
...
void SBA::init(unsigned int maxImages, unsigned int maxNumResiduals) {
...
void SBA::setGlobalWeights(const std::vector<float>& weightsSparse, const std::vector<float>& weightsDenseDepth, const std::vector<float>& weightsDenseColor, bool useGlobalDenseOpt) {
	m_globalWeightsMutex.lock();
...
bool SBA::align(SIFTImageManager* siftManager, const CUDACache* cudaCache, float4x4* d_transforms, 
	unsigned int maxNumIters, unsigned int numPCGits, bool useVerify, bool isLocal,
	bool recordConvergence, bool isStart, bool isEnd, bool isScanDoneOpt, unsigned int revalidateIdx /*= (unsigned int)-1*/)
{
	...
	if (isLocal) { //局部优化weight设置
		...
		if (!m_bUseGlobalDenseOpt) { //不使用global dense优化
			cache = NULL;
			weightsDenseDepth = std::vector<float>(m_globalWeightsDenseDepth.size(), 0.0f); //全置为0
			weightsDenseColor = weightsDenseDepth;
		}
		else { //扫描结束时，才调用global dense优化（默认）
			m_globalWeightsMutex.lock();
			weightsDenseDepth = m_globalWeightsDenseDepth;
			weightsDenseColor = m_globalWeightsDenseColor;
			m_globalWeightsMutex.unlock();
			std::cout << "Global dense optimization.\n";
		}
	}
	...
	bool removed = alignCUDA(siftManager, cache, usePairwise, 
		weightsSparse, weightsDenseDepth, weightsDenseColor, maxNumIters, numPCGits, isStart, isEnd, revalidateIdx);
	...
	if (!isScanDoneOpt && GlobalBundlingState::get().s_enableGlobalTimings) { 
		cudaDeviceSynchronize(); s_timer.stop(); TimingLog::getFrameTiming(isLocal).timeSolve += s_timer.getElapsedTimeMS(); 
		TimingLog::getFrameTiming(isLocal).numItersSolve += maxNumIters; 
	}
	return removed;
}

bool SBA::alignCUDA(SIFTImageManager* siftManager, const CUDACache* cudaCache, bool useDensePairwise, 
