...
{
	m_depthIntrinsics = mat4f(	
		fovX, 0.0f, centerX, 0.0f,
...
void RGBDSensor::recordFrame()
{
	if (m_bUseModernSensFilesForRecording) {
		...
		//std::free(color); 
		//std::free(depth);
		m_recordedDataCache->writeNextAndFree(color, depth, m_timeStampColor, m_timeStampDepth);
	}
	else {
...
void RGBDSensor::saveRecordedFramesToFile(std::string& filename, const std::vector<mat4f>& trajectory, bool overwriteExistingFile /*= false*/)
{
	if (m_bUseModernSensFilesForRecording) { //.sens格式
		...
		unsigned int numFrames = (unsigned int)trajectory.size();//有效pose的frame数
		unsigned int numRecordedFrames = (unsigned int)m_recordedData->m_frames.size();////记录的frame数
		numFrames = std::min(numFrames, numRecordedFrames);
		...
		//free frames without a trajectory 去掉多余的frames
		if (numFrames < numRecordedFrames) {
			for (size_t i = numFrames; i < numRecordedFrames; i++) {
				m_recordedData->m_frames[i].free();
			}
			m_recordedData->m_frames.resize(numFrames);
		}
		std::cout << "有效pose的frame数:" << numFrames << ",记录的frame数:" << numRecordedFrames << std::endl;

		//std::cout << *m_recordedData << std::endl;
		std::cout << "dumping recorded frames ... ";

		std::string actualFilename = filename;
		if (!overwriteExistingFile) {//产生文件名
