...
void SensorDataReader::createFirstConnected()
{
	...
	//m_numFrames = 318; //test
	std::cout << "sens file #frames = " << m_numFrames << std::endl;
	...
	//panrj
	std::string poseFileName = util::removeExtensions(filename)+".pose";
	if (util::fileExists(poseFileName))
	{
		m_hasPoseSensor = true;
		//从文件中读取t265相机位姿
		std::ifstream infile;
		infile.open(poseFileName);
		for (int i = 0; i < m_numFrames; i++) {
			//从文件中读取mat4f类型的矩阵
			mat4f pose;
			infile >> pose;
			m_vImuPose.push_back(pose);
		}
		infile.close();
	}
}
...
bool SensorDataReader::processDepth()
{
	...
	//	return false;
	//}
	return GlobalAppState::get().s_playData;
}
