...
struct BundlerInputData {
	...
	float*					d_intensityFilterHelper; //TODO check if used, m_bFilterIntensity为true时，才需要
	...
	void alloc(const RGBDSensor* sensor) {
		...
		if (m_bFilterIntensity) MLIB_CUDA_SAFE_CALL(cudaMalloc(&d_intensityFilterHelper, sizeof(float)*m_widthSIFT*m_heightSIFT));
	}
	...
	~BundlerInputData() {
		...
		if (m_bFilterIntensity) MLIB_CUDA_SAFE_CALL(cudaFree(d_intensityFilterHelper));
	}
};
...
struct BundlerState {
	...
	//current frames
	int						m_iLastFrameProcessed;
	bool					m_bLastFrameValid;

	int						m_iLocalToSolve;		// index of local submap to solve (-1) if none
	...
	unsigned int			m_iLastValidCompleteTransform;
	bool					m_bGlobalTrackingLost;

	PROCESS_STATE			m_processState;
	bool					m_bUseSolve; //是否优化
