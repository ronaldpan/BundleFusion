...
void TrajectoryManager::generateUpdateLists()
{
	...
	for (unsigned int i = 0; i < numFrames; i++) {
		...
		if (f.optimizedTransform[0] == -std::numeric_limits<float>::infinity()) {//无效变换
			//f.type = TrajectoryFrame::Invalid;
			//invalidateFrame(i);	//adding it to the deIntragte list. copy到下面
			if (f.type != TrajectoryFrame::Invalid)
			{
				TrajectoryFrame::TYPE typeBefore = f.type;
				f.type = TrajectoryFrame::Invalid;
				if (typeBefore == TrajectoryFrame::Integrated) {
					m_toDeIntegrateList.push_back(&f);
				}
			}
		}
		else {
	...
	auto s = [](const TrajectoryFrame *left, const TrajectoryFrame *right) {
		...
		return left->dist > right->dist;//left和right都是Integrated
	};
	//std::sort(m_framesSort.begin(), m_framesSort.begin() + numFrames, s);
	std::nth_element(begin(m_framesSort), begin(m_framesSort) + m_topNActive -1, end(m_framesSort), s); //panrj
	//m_framesSort.sort(s);
...
bool TrajectoryManager::getTopFromToIntegrateList(mat4f& trans, unsigned int& frameIdx)
{
...
void TrajectoryManager::getOptimizedTransforms(std::vector<mat4f>& transforms) {
	m_mutexUpdateTransforms.lock();
