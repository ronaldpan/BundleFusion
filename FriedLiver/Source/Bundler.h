...
public:
	...
	void storeCachedFrame(const float* d_inputDepthRaw, unsigned int depthWidth, unsigned int depthHeight, 
						  const uchar4* d_inputColor, unsigned int colorWidth, unsigned int colorHeight);
	...
	unsigned int matchAndFilter(unsigned int curFrame= (unsigned int)-1);
	...
	//}
	SBA & getOptimizer() { return m_optimizer; }
...
private:
	...
	bool					m_bIsLocal;//¿éÄÚintra_chunk: true, ¿é¼äinter_chunk: false 
	Timer					m_timer;
