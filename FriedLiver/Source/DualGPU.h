...
public:
	...
	private:
		int m_deviceIndex;
		bool m_bIsEmulated; //true:物理GPU，false:模拟GPU
	};
...
private:
	...
	void init(unsigned int minDevices = 1, unsigned int maxPhysicalDevices = 1)
	{
		unsigned int numDevices;//物理GPU数
		...
		assert(m_gpus.size() > 0);//必须有GPU
		const GPU& last = m_gpus.back(); //最后的物理GPU
		for (unsigned int i = numDevices; i < minDevices; i++) {	//fill up with emulated devices
			m_gpus.push_back(GPU(last.getDeviceIdx(), true)); //模拟GPU
		}
		printList();
		m_bIsInit = true;
	}
