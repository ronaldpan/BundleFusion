...
CBaseCamera::CBaseCamera()
{
	...
	m_vMyKeyboardDirection.x = m_vMyKeyboardDirection.y = m_vMyKeyboardDirection.z = 0.0f;
	D3DXMatrixIdentity(&m_mShift);
}
...
VOID CBaseCamera::SetViewParams( D3DXVECTOR3* pvEyePt, D3DXVECTOR3* pvLookatPt )
{
	...
	//by simanies initializes the translation parameters
	m_vMyKeyboardDirection.x = m_vMyKeyboardDirection.y = m_vMyKeyboardDirection.z = 0.0f;
	D3DXMatrixIdentity(&m_mShift);
}
...
void CModelViewerCamera::SetViewParams( D3DXVECTOR3* pvEyePt, D3DXVECTOR3* pvLookatPt )
{
    ...
    SetRadius( D3DXVec3Length( &vEyeToPoint ), 0.2f );
...
LRESULT CModelViewerCamera::HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    if( uMsg == WM_MOUSEMOVE )
    {
		...
		if (m_nZoomButtonMask & MOUSE_RIGHT_BUTTON && m_bMouseRButtonDown) {
            FLOAT fDeltaX = (FLOAT)( m_ptLastMouseX - iMouseX );
            //FLOAT fDeltaY = (FLOAT)( m_ptLastMousePosition.y - iMouseY );
			//m_fRadius -= fDeltaX * m_fRadius * 0.1f / 120.0f;
			m_fRadius -= fDeltaX * m_fRadius / 100.0f;
		}
		m_ptLastMouseX = iMouseX;
    }
